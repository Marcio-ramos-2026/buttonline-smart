...
export async function createClientAction(data: CreateAdminType) {
  const t = await getTranslations("pages.generalApiReturns");

  if (!(await hasPermission([ALLOWED_PERMISSIONS.ADMIN_CLIENTS_CREATE]))) {
  ...
  try {
    ...
    await prisma.user.create({
      data: {
        ...
        role: {
          connect: {
            id: 2, //always register with a non admin role
          },
        },
      },
    });
    return {
      success: true,
      message: t("clients.createdSuccess"),
    };
  } catch (e) {
      ...
      switch (e.code) {
        case "P2002":
          if (e.meta && e.meta?.target === "users_email_key") {
            return { error: t("clients.emailInUse") };
          }
          break;
        default:
          return {
            error: t("clients.createdError"),
          };
      }
    }
